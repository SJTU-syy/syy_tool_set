...
class BodyTab(QWidget):
    ...
    # 创建 Root 骨骼的平移控制器
    def add_root_translation_controller(fit_skeleton):
        # 获取名为 "Root" 的骨骼
        root_bone = cmds.ls(fit_skeleton + "|Root", type="transform")
        if not root_bone:
            cmds.error("Root 骨骼未找到")
            return

        root_bone = root_bone[0]

        # 创建控制器
        controller = cmds.circle(name="Root_Controller", normal=(0, 1, 0), radius=1)[0]

        # 将控制器移动到 Root 骨骼的位置
        root_position = cmds.xform(root_bone, query=True, translation=True, worldSpace=True)
        cmds.xform(controller, translation=root_position, worldSpace=True)

        # 将控制器约束到 Root 骨骼
        cmds.parentConstraint(controller, root_bone, maintainOffset=True)

        # 创建控制器的属性
        cmds.addAttr(controller, longName="TranslateX", attributeType="double", defaultValue=0, keyable=True)
        cmds.addAttr(controller, longName="TranslateY", attributeType="double", defaultValue=0, keyable=True)

        # 连接控制器的属性到 Root 骨骼的平移
        cmds.connectAttr(controller + ".TranslateX", root_bone + ".translateX")
        cmds.connectAttr(controller + ".TranslateY", root_bone + ".translateY")
